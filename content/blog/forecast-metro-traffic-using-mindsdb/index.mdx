---
slug: 'forecast-metro-traffic-using-mindsdb'
title: 'Forecast Metro Traffic using MindsDB Cloud and MongoDB Atlas'
date: '2021-10-19'
author: 'Aditya DS'
published: true
...
👉 Train a model to predict metro traffic.

👉 Get a prediction from the model given certain input parameters.

All of that **without writing a single line of code and in less than 15 minutes**. Yes, you read that right! 💯

We will be using the Metro traffic dataset 🚇 that can be downloaded from [here](https://github.com/mindsdb/benchmarks/blob/main/benchmarks/datasets/metro_traffic_ts/data.csv). You are also free to use your own dataset and follow along the tutorial.
...
3. You are also expected to have an account on MindsDB Cloud. If not, head over to [https://cloud.mindsdb.com/](https://cloud.mindsdb.com/) and create an account. It hardly takes a few minutes. ⚡
...
Phew! With all of that out of the way, we can finally get started! 🚀
...
1. Given a certain `traffic_volume` what is the probability the sky is clear ☀️? What is the probability that it is raining? 🌧️
2. Given a certain `traffic_volume`, how certain can we be that the day is a holiday? 🏖️
3. Can we predict more parameters instead of only the `traffic_volume`? 🤔  
